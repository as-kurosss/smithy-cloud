"""One-shot scheduled triggers: CRUD + firing due triggers into process runs."""
...
def _trigger_status(trigger: Trigger) -> Literal["scheduled", "fired", "disabled"]:
    if trigger.fired_at is not None:
        return "fired"
...
async def fire_due_triggers(db: AsyncSession) -> int:
    ...
    Late triggers fire on the next poll — a trigger is never skipped.
